...
def evaluation(
    ...
    use_dposer=True,  # 新增参数
    dposer_weight=0.05,  # 新增参数
):
    ...
    for (idx,), (motion_rep, model_kwargs) in tqdm(
        ...
        # start optimizing - 重要修改：传入diffusion对象给DNO
        noise_opt = DNO(
            model=solver, 
            criterion=criterion, 
            start_z=cur_xt, 
            conf=dno_conf,
            diffusion=diffusion  # 传入diffusion对象用于DPoser
        )
...
if __name__ == "__main__":
    ...
    conf = DNOOptions(
        ...
        dposer_timestep_strategy='truncated',  # 'random', 'fixed', 'truncated'
    ...
    evaluation(
        ...
        use_gradient_checkpoint=False,
        debug=True,
        use_dposer=USE_DPOSER,  # 传递DPoser配置
        dposer_weight=DPOSER_WEIGHT,
    )