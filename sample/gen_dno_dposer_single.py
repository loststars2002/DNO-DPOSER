# gen_dno_dposer_single.py
"""
Single-step denoising version using original DPoser style
python -m sample.gen_dno_dposer_single --model_path ./save/mdm_avg_dno/model000500000_avg.pt --text_prompt "a person is jumping"
"""
import json
import os
import pickle
...
from dno_single import DNO, DNOOptions  # 使用单步去噪版本
...
def main(num_trials=3):
    ...
    # DPoser 配置（单步去噪版本）
    USE_DPOSER = True  # 设置为False可以禁用DPoser正则化
    DPOSER_WEIGHT = 0.05  # DPoser正则化权重，建议从0.01-0.1范围调整
    ...
    print("##### Task: %s #####" % task)
    if USE_DPOSER:
        print(f"##### DPoser regularization enabled with weight {DPOSER_WEIGHT} (Single-step denoising) #####")
    #############################################
    args = generate_args()
    args.device = 0
    # print(args.__dict__)
    ...
    # 根据是否使用DPoser修改输出目录名
    suffix = "dno_dposer_single" if USE_DPOSER else "dno"
    out_path = os.path.join(out_path, task + "_" + suffix)
    ...
    # 获取底层扩散模型（用于单步去噪）
    if args.guidance_param != 1:
        base_diffusion_model = model.model  # ClassifierFreeSampleModel的底层模型
    else:
        base_diffusion_model = model
    
    print(f"Base diffusion model type: {type(base_diffusion_model)}")
    ###################################
    collate_args = [
        {
    ...
    for rep_i in range(args.num_repetitions):
        ...
        noise_opt_conf = DNOOptions(
            ...
            # DPoser 配置参数（单步去噪版本）
            dposer_penalty_scale=DPOSER_WEIGHT if USE_DPOSER else 0.0,
            dposer_timestep_strategy='truncated',  # 推荐使用 'truncated'
            dposer_use_snr_weighting=True,        # 启用SNR权重
            dposer_t_max=0.15,
            dposer_t_min=0.05,
            dposer_t_fixed=0.1,  # 固定时间步（当strategy='fixed'时使用）
        )
        ...
        ######## Main optimization loop (单步去噪版本) #######
        noise_opt = DNO(
            model=solver, 
            criterion=criterion, 
            start_z=cur_xt, 
            conf=noise_opt_conf,
            diffusion=diffusion,                  # 传入diffusion对象
            base_model=base_diffusion_model       # 关键：传入底层模型用于单步去噪
        )
        #######################################
        out = noise_opt()
        ...
        for i in range(num_trials):
            ...
            # 如果使用DPoser，添加DPoser损失到绘图
            if USE_DPOSER:
                loss_keys.append("loss_dposer")
                
            for key in loss_keys:
                plt.figure()
                if key in ["loss", "loss_diff", "loss_decorrelate", "loss_dposer"]:
                ...
                plt.title(f"Trial {i} - {key} (Single-step DPoser)")
                plt.legend([key])
        ...
        if task == "motion_blending":
            ...
            captions = [
                ...
                "Original 2",
                "Naive concatenation",
            ] + [f"Prediction {i+1} (Single-step)" for i in range(num_trials)]
    ...
    print(f"[Note] Used single-step denoising DPoser (original style)")
