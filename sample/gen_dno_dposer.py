# gen_dno_dposer.py
""""
python -m sample.gen_dno_dposer --model_path ./save/mdm_avg_dno/model000500000_avg.pt --text_prompt "a person is jumping"
...
def main(num_trials=3):
    ...
    DPOSER_WEIGHT = 0.01  # DPoser正则化权重，建议从0.01-0.1范围调整
    ...
    print("##### Task: %s #####" % task)
    if USE_DPOSER:
        print(f"##### DPoser regularization enabled with weight {DPOSER_WEIGHT} #####")
    #############################################
    args = generate_args()
    args.device = 0
    # print(args.__dict__)
    ...
    # 根据是否使用DPoser修改输出目录名
    suffix = "dno_dposer" if USE_DPOSER else "dno"
    ...
    for rep_i in range(args.num_repetitions):
        ...
        noise_opt_conf = DNOOptions(
            ...
            dposer_timestep_strategy='fixed',  # 推荐使用 'fixed'
            dposer_t_max=0.15,
            dposer_t_min=0.05,
            dposer_t_fixed=0.1,  # 固定时间步
        )
