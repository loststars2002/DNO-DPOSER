...
class DNO:
    """
    DNO with single-step denoising DPoser (like original DPoser)
    Args:
        start_z: (N, 263, 1, 120)
    """
    ...
    def __init__(
        self,
        model,              # solver函数（完整DDIM采样）
        criterion,          # 任务损失函数
        start_z,            # 初始噪声
        conf: DNOOptions,   # 配置
        diffusion=None,     # diffusion对象用于噪声调度
        base_model=None,    # 底层扩散模型用于单步去噪
    ):
        self.model = model  # solver函数
        self.criterion = criterion
        self.diffusion = diffusion
        self.base_model = base_model  # 新增：底层扩散模型
        ...
        if self.use_dposer:
            ...
            print(f"DPoser mode: Single-step denoising (original style)")
            self.dposer_loss_fn = torch.nn.MSELoss(reduction='none')
            
            if self.base_model is None:
                print("Warning: base_model is None, DPoser will be disabled")
                self.use_dposer = False
            else:
    ...
    def _setup_dposer_schedule(self):
        ...
        if self.diffusion is not None:
            ...
            print("DPoser: Using diffusion object's noise schedule")
        else:
    ...
    def _get_noise_schedule_params(self, t_val):
        ...
        return sqrt_alpha_cumprod, sqrt_one_minus_alpha_cumprod, alpha_cumprod, timestep_idx

    def compute_dposer_loss(self, x_0, step):
        """
        原版DPoser风格的单步去噪正则化损失
        Args:
        ...
        """
        if not self.use_dposer or self.base_model is None:
        ...
        if self.conf.dposer_timestep_strategy == 'random':
            eps = 1e-3
            t_val = eps + torch.rand(1).item() * (self.conf.dposer_t_max - self.conf.dposer_t_min - eps)
        ...
        # 获取精确的噪声调度参数
        sqrt_alpha_cumprod, sqrt_one_minus_alpha_cumprod, alpha_cumprod, timestep_idx = self._get_noise_schedule_params(t_val)
        ...
        with torch.no_grad():
            try:
                # 创建时间步张量
                t_tensor = torch.full((batch_size,), timestep_idx, device=device, dtype=torch.long)
                
                # 调用底层模型预测噪声 ε_θ(x_t, t)
                predicted_noise = self.base_model(x_t, t_tensor)
                
                # 单步去噪公式（原版DPoser）: x̂_0 = (x_t - √(1-ᾱ_t) * ε̂) / √ᾱ_t
                x_0_hat = (x_t - sqrt_one_minus_alpha_cumprod * predicted_noise) / sqrt_alpha_cumprod
                
            except Exception as e:
                print(f"Warning: DPoser single-step denoising failed: {e}")
    ...
    def __call__(self, num_steps: int = None):
        ...
        with tqdm(range(num_steps)) as prog:
            for i in prog:
                ...
                # DPoser regularization (single-step denoising)
