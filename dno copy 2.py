...
class DNO:
    ...
    def compute_dposer_loss(self, x_0, step):
        ...
        if self.conf.dposer_timestep_strategy == 'random':
            ...
            progress = step / self.conf.num_opt_steps
            t_val = self.conf.dposer_t_max - progress * (self.conf.dposer_t_max - self.conf.dposer_t_min)
            t_val = max(t_val, self.conf.dposer_t_min)
        else:
        ...
        # 简化的噪声调度
        alpha_t = 1.0 - t_val  # 在t=0时alpha=1，t=1时alpha=0
        sigma_t = t_val        # 在t=0时sigma=0，t=1时sigma=1
        
        # 加噪: x_t = alpha_t * x_0 + sigma_t * noise  
        x_t = alpha_t * x_0 + sigma_t * noise
        
        # 使用现有模型进行去噪估计 (stop gradient)
        with torch.no_grad():
            try:
                # 直接使用现有的model进行去噪
        ...
        # 计算DPoser损失: ||x_0 - sg[x̂_0(t)]||²_2
        # 这里x_0_hat已经通过torch.no_grad()停止了梯度
        dposer_loss = torch.mean(
            self.dposer_loss_fn(x_0, x_0_hat), 
            dim=[1, 2, 3]  # 对除batch维度外的所有维度求平均
        )
    ...
    def __call__(self, num_steps: int = None):
        ...
        with tqdm(range(num_steps)) as prog:
            for i in prog:
                ...
                if self.conf.dposer_penalty_scale > 0:
                    dposer_val = info["loss_dposer"].mean().item() if isinstance(info["loss_dposer"][0], torch.Tensor) else 0
